...
// (0.375f,0.375f)
void
ImGui_ImplGlfwGL3_RenderDrawLists(struct ImDrawData* draw_data) {
  ...
  for (n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    int vertexBufferSize = ImDrawList_GetVertexBufferSize(cmd_list);
    struct ImDrawVert* vertexBufferData =
        ImDrawList_GetVertexBufferData(cmd_list);

    glBindBuffer(GL_ARRAY_BUFFER, g_VboHandle);
    glBufferData(GL_ARRAY_BUFFER,
                 (GLsizeiptr)vertexBufferSize * sizeof(struct ImDrawVert),
                 (const GLvoid*)vertexBufferData, GL_STREAM_DRAW);

    int indexBufferSize = ImDrawList_GetIndexBufferSize(cmd_list);
    ImDrawIdx* indexBufferData = ImDrawList_GetIndexBufferData(cmd_list);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, g_ElementsHandle);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 (GLsizeiptr)indexBufferSize * sizeof(ImDrawIdx),
                 (const GLvoid*)indexBufferData, GL_STREAM_DRAW);

    int cmdBufferSize = ImDrawList_GetCmdSize(cmd_list);
    int cmd_i;
    for (cmd_i = 0; cmd_i < cmdBufferSize; cmd_i++) {
      struct ImDrawCmd* pcmd = ImDrawList_GetCmdPtr(cmd_list, cmd_i);
...
static const char*
ImGui_ImplGlfwGL3_GetClipboardText(void* user_data) {
  return glfwGetClipboardString((GLFWwindow*)user_data);
}

static void
ImGui_ImplGlfwGL3_SetClipboardText(void* user_data, const char* text) {
  glfwSetClipboardString((GLFWwindow*)user_data, text);
}

void
ImGui_ImplGlfwGL3_MouseButtonCallback(struct GLFWwindow* window, int button,
                                      int action, int mods) {
...
void
ImGui_ImplGlfwGL3_ScrollCallback(struct GLFWwindow* window, double xoffset,
                                 double yoffset) {
  g_MouseWheel += (float)yoffset; // Use fractional mouse wheel.
}

void
ImGui_ImplGlfwGL3_KeyCallback(struct GLFWwindow* window, int key, int scancode,
                              int action, int mods) {
  struct ImGuiIO* io = igGetIO();
...
void
ImGui_ImplGlfwGL3_CharCallback(struct GLFWwindow* window, unsigned int c) {
  if (c > 0 && c < 0x10000)
...
ImGui_ImplGlfwGL3_CreateFontsTexture() {
  ...
  // If your ImTextureId represent a higher-level concept than just a GL texture
  // id, consider calling GetTexDataAsAlpha8()
  ...
  ImFontAtlas_SetTexID(io->Fonts, (void*)(intptr_t)g_FontTexture);
...
#define OFFSETOF(TYPE, ELEMENT) ((size_t) & (((TYPE*)0)->ELEMENT))
  glVertexAttribPointer(g_AttribLocationPosition, 2, GL_FLOAT, GL_FALSE,
                        sizeof(struct ImDrawVert),
                        (GLvoid*)OFFSETOF(struct ImDrawVert, pos));
  glVertexAttribPointer(g_AttribLocationUV, 2, GL_FLOAT, GL_FALSE,
                        sizeof(struct ImDrawVert),
                        (GLvoid*)OFFSETOF(struct ImDrawVert, uv));
  glVertexAttribPointer(g_AttribLocationColor, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                        sizeof(struct ImDrawVert),
                        (GLvoid*)OFFSETOF(struct ImDrawVert, col));
#undef OFFSETOF
...
bool
ImGui_ImplGlfwGL3_Init(GLFWwindow* window, bool install_callbacks) {
  g_Window = window;
...
ImGui_ImplGlfwGL3_NewFrame() {
  ...
  io->DisplayFramebufferScale.x = w > 0 ? ((float)display_w / (float)w) : 0.0f;
  io->DisplayFramebufferScale.y = h > 0 ? ((float)display_h / (float)h) : 0.0f;
  ...
  if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED)) {
    if (io->WantMoveMouse) {
      glfwSetCursorPos(g_Window, (double)io->MousePos.x,
                       (double)io->MousePos.y); // Set mouse position if
                                                // requested by
                                                // io->WantMoveMouse flag (used
                                                // when io->NavMovesTrue is
                                                // enabled by user and using
                                                // directional navigation)
    } else {
      double mouse_x, mouse_y;
      glfwGetCursorPos(g_Window, &mouse_x, &mouse_y);
      io->MousePos = (struct ImVec2){
          (float)mouse_x, (float)mouse_y}; // Get mouse position in screen

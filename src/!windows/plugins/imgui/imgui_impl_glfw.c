...
//  2018-11-07: Inputs: When installing our GLFW callbacks, we save user's previously installed ones
//  - if any - and chain call them. 2018-08-01: Inputs: Workaround for Emscripten which doesn't seem
//  to handle focus related calls. 2018-06-29: Inputs: Added support for the ImGuiMouseCursor_Hand
//  cursor. 2018-06-08: Misc: Extracted imgui_impl_glfw.cpp/.h away from the old combined
//  GLFW+OpenGL/Vulkan examples. 2018-03-20: Misc: Setup io->BackendFlags
...
//  ImGuiConfigFlags_NavEnableGamepad is set. 2018-01-25: Inputs: Honoring the io->WantSetMousePos
//  by repositioning the mouse (when using navigation and ImGuiConfigFlags_NavMoveMouse is set).
...
enum GlfwClientApi {
  GlfwClientApi_Unknown = 0,
  GlfwClientApi_OpenGL = 1,
  GlfwClientApi_Vulkan = 2
};
...
static bool
ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, enum GlfwClientApi client_api) {
  g_Window = window;
  g_Time = 0.0;

...
hammer_init(r_app_t* app, r_api_db_i* api_db) {
  ...
  r_logger_file_device_i* file_device =
      api_db->find_by_name(api_db->instance, R_LOGGER_FILE_DEVICE_API_NAME);

  this->debug_api->add_device(outputstring_device);
  this->debug_api->add_device((r_logger_device_i*)file_device);

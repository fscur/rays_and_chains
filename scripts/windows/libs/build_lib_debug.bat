...
pushd %ROOT%\
.\tools\pdb_path_fixer\pdb_path_fixer.exe ".\build\libs\%LIB_NAME%\%LIB_NAME%.dll"
popd 

call log [info] "Copying output to bin"
pushd p:\
robocopy .\build\libs\%LIB_NAME% .\bin\libs %LIB_NAME%.*.pdb /xo /njh /njs /ndl /nc /ns /nfl
robocopy .\build\libs\%LIB_NAME% .\bin\libs %LIB_NAME%.dll /xo /njh /njs /ndl /nc /ns /nfl
popd